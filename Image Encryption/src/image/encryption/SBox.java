 ...
 */
public class SBox {
    //standard table for AES algorithm do not change it is fixed
    ...
    //change the state into sub bytes
    ...
    public static int[] substituteCol(int[] state){
        for(int i=0;i<state.length;i++){
            int hex=state[i];
            state[i]=sbox[hex/16][hex%16];
        }
        return state;
    }
